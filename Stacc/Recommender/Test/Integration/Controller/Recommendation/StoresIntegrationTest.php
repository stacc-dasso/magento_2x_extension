...
class StoresIntegrationTest extends AbstractController
{
    ...
    {
        $this->dispatch('recommender/recommendation/stores?h='.hash("sha256", $this::TEST_API_KEY));
        $this->assertEquals("", $this->getResponse()->getBody());
        $this->dispatch('recommender/recommendation/stores?h='.hash("sha256", $this::TEST_SHOP_ID));
        $this->assertEquals("", $this->getResponse()->getBody());
    }
}
