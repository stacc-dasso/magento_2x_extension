...
class CheckIntegrationTest extends AbstractController
{
    ...
    public function testRendersSuccessfulCheckVerificationResponse()
    {
        $time = time();
        $this->dispatch('recommender/recommendation/check?h=' . hash("sha256", $this::TEST_SHOP_ID . $this::TEST_API_KEY) . "&t=" . $time);
    ...
    {
        $this->dispatch('recommender/recommendation/check?h=' . hash("sha256", $this::TEST_API_KEY));
        $this->assertEquals("", $this->getResponse()->getBody());
        $this->dispatch('recommender/recommendation/check?h=' . hash("sha256", $this::TEST_SHOP_ID));
        $this->assertEquals("", $this->getResponse()->getBody());
    }
}
